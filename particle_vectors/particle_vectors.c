...
int main(void)
{
    ...
    InitWindow(screenWidth, screenHeight, "Particle Soup");
    ...
    while (!WindowShouldClose()) // Detect window close button or ESC key
    {
        ...
        for (int i = 0; i < MAX_PARTICLES; i++)
        {
            ...
            if (particles[i].position.x <= particles[i].radius)
            {
                ...
                edge_position = (Vector2){0.0, particles[i].position.y};
                on_edge = true;
            }
            // Right edge
            ...
            {
                particles[i].velocity.x *= -1;
                edge_position = (Vector2){screenWidth, particles[i].position.y};
                on_edge = true;
            }
            // Top edge
            if (particles[i].position.y <= particles[i].radius)
            {
                ...
                edge_position = (Vector2){particles[i].position.x, 0.0};
                on_edge = true;
            }
            // Bottom edge
            ...
            {
                particles[i].velocity.y *= -1;
                edge_position = (Vector2){particles[i].position.x, screenHeight};
                on_edge = true;
            }
